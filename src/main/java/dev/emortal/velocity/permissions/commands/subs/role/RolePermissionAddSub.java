...
public final class RolePermissionAddSub implements EmortalCommandExecutor {
    ...
    public void execute(@NotNull CommandSource source, @NotNull ArgumentProvider arguments) {
        ...
        if (oldState == newState) {
            ChatMessages.ERROR_PERMISSION_ALREADY_SET.send(source, roleId, permission, newValue);
            return;
        }
        ...
        switch (result) {
            case UpdateRoleResult.Success(Role newRole) -> {
                ...
                ChatMessages.PERMISSION_ADDED_TO_ROLE.send(source, permission, newValue, roleId);
            }
